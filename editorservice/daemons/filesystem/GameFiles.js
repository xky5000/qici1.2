...
function _saveUrlMap()
{
    ...
    fsExtra.writeFileSync(path.join(G.gameRoot, 'Assets/meta/globalUrlMap.js'),
        'window.urlMapConfig = ' + JSON.stringify(urlMap, null, 2) + ';\n');

    if (G.config.project.subDomain) {
        // 如果为小游戏子域工程，需要将 bin 资源保存到 assetDataMap.js 中
        var assetDataMap = {};
        for (var uuid in urlMap) {
            var url = urlMap[uuid];
            if (url.indexOf('Assets') !== 0)
                // 只保存 Assets 目录下的 .bin 文件
                continue;
            var content = fsExtra.readFileSync(path.join(G.gameRoot,url), 'utf8');
            assetDataMap[url] = content;
        }
        // 写入文件
        fsExtra.writeFileSync(path.join(G.gameRoot, 'Assets/meta/assetDataMap.js'),
            'window.assetDataMap = ' + JSON.stringify(assetDataMap, null, 2) + ';\n');
    }
