...
ServerCommunicate.prototype.reset = function() {
    var self = this;

    self.socket = null;
    self.isConnected = false;
    self.lpcConnected = false;
    self._socketCmdMap = {};
    self.onConnect = new Phaser.Signal();
    self.onDisconnect = new Phaser.Signal();
    self.onError = new Phaser.Signal();
}

/**
 * 断开连接
 */
ServerCommunicate.prototype.destroy = function() {
    var self = this;
    if (self.socket) {
 ...
 * @param func {string} - 处理函数
 */
ServerCommunicate.socketConnect = function(node, connectCallback, opts) {
    var serverCommunicate = node.getScript('qc.ServerCommunicate');
    serverCommunicate.onConnect.add(connectCallback);
